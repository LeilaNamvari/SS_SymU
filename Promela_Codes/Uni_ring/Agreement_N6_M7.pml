...
#define localInv(i)  (x[dec(i)] == x[i])
#define globalInv    (localInv(0) && localInv(1) && localInv(2) && localInv(3) && localInv(4) && localInv(5))
...
active [N] proctype P(){
  ...
  int i = _pid;
do 	 
        :: atomic{(x[dec(i)] >= 0) && (x[dec(i)] != x[i]) && (x[i] != 1) -> x[i] = 1}
od; 
}
