...
#define globalInv (localInv(1)&&localInv(2)&&localInv(3)&&localInv(4)&&localInv(5)&&localInv(6))
...
active [N] proctype P() {
 int i = _pid; 
  ...
  :: atomic{ (i>0) && (x[Parent(i)] >= 2)  && (isEven(x[Parent(i)]))  &&
   (NEvenParity(x[Parent(i)], x[i])) && (x[i] != 0) -> x[i] = 0 }
   od
